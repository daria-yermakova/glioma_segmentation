            ...
            if index == 2 and (not is_sweep):
                for batch_index in range(0, len(inputs)):
                    ...
                    for index_pred in range(0, len(logits)):
                        ...
                        print(f'pred {index_pred}',  np_pred.sum(), np.sum(np_pred), len(np.unique(np_pred)) <= 2)
                        matched_pixels = np.where((np_target > 0) & (np_pred > 0.5), 1, 0)
                        print('matched_pixels', matched_pixels.sum(), np.sum(matched_pixels),
                              len(np.unique(matched_pixels)) <= 2)

                        np.save(f"{path}/pred/pred_e{epoch}_b{batch_index}_{index_pred}.npy", np_pred)
